...
uint32_t ledcSetup(uint8_t chan, uint32_t freq, uint8_t bit_num)
{
    if(chan >= LEDC_CHANNELS || bit_num > LEDC_MAX_BIT_WIDTH){
        //log("No more LEDC channels available! (maximum %u) or bit width too big (maximum %u)", LEDC_CHANNELS, LEDC_MAX_BIT_WIDTH);
        return 0;
    }
...
{
    gpio_iomux_out(pin, false, false);
...
{
    if(chan >= LEDC_CHANNELS || bit_num > LEDC_MAX_BIT_WIDTH){
        //log("LEDC channel not available! (maximum %u) or bit width too big (maximum %u)", LEDC_CHANNELS, LEDC_MAX_BIT_WIDTH);
        return 0;
    }
    uint8_t group=(chan/8), timer=((chan/2)%4);
    ...
    if(ledc_timer_config(&ledc_timer) != ESP_OK)
    {
        //log("ledcChangeFrequency failed!");
        return 0;
    }
    channels_resolution[chan] = bit_num;
    return ledc_get_freq(group,timer);
...
void analogWrite(uint8_t pin, int value) {
    ...
    if (pin < SOC_GPIO_PIN_COUNT) {
        ...
        if (pin_to_channel[pin] == 0) {
            if (!cnt_channel) {
                //log("No more analogWrite channels available! You can have maximum %u", LEDC_CHANNELS);
                return;
            }
            cnt_channel--;
            channel = cnt_channel;
        ...
        //log("GPIO %d - Using Channel %d, Value = %d", pin, channel, value);
        if(ledcSetup(channel, analog_frequency, analog_resolution) == 0){
            //log("analogWrite setup failed (freq = %u, resolution = %u). Try setting different resolution or frequency");
            return;
        }
    ...
    if (cnt_channel != LEDC_CHANNELS) {
        for (int channel = LEDC_CHANNELS - 1; channel >= cnt_channel; channel--) {
            if (ledcChangeFrequency(channel, freq, analog_resolution) == 0){
                //log("analogWrite frequency cant be set due to selected resolution! Try to adjust resolution first");
                return;
            }
        }
    }
...
void analogWriteResolution(uint8_t bits) {
    if(bits > LEDC_MAX_BIT_WIDTH) {
        //log("analogWrite resolution width too big! Setting to maximum %u bits)", LEDC_MAX_BIT_WIDTH);
        bits = LEDC_MAX_BIT_WIDTH;
    }
    if (cnt_channel != LEDC_CHANNELS) {
        for (int channel = LEDC_CHANNELS - 1; channel >= cnt_channel; channel--) {
            if (ledcChangeFrequency(channel, analog_frequency, bits) == 0){
                //log("analogWrite resolution cant be set due to selected frequency! Try to adjust frequency first");
                return;
            }
        }
    }
